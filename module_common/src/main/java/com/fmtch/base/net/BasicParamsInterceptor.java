...
public class BasicParamsInterceptor
	...
	public Response intercept(Chain chain)
		...
		try {
			Gson gson = new Gson();
			com.fmtch.base.net.response.Response<SuperResponse> superResponse = gson.fromJson(buffer.clone().readString(Charset.forName("UTF-8")), com.fmtch.base.net.response.Response.class);
			if (200 != superResponse.getCode() && !TextUtils.isEmpty(superResponse.getMessage())) {
				if (401 == superResponse.getCode()) {
					//登录失效重新登录
					EventBus.getDefault().post(new EventBean<>(EventType.USER_EXIT));
					ARouter.getInstance().build(RouterMap.ACCOUNT_LOGIN).navigation();
				}
				ToastUtils.showLongToast(superResponse.getMessage());
			}
		} catch (Exception e) {
		...
		if (BuildConfig.DEBUG) {
			KLog.d("url : " + request.url().toString());
			//使用前clone()下，避免直接消耗
