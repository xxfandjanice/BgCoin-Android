...
public class BlackListModel {
    ...
    private BlackListFragment view;
    private int page = 1;
    private int limit = 15;

    public BlackListModel(BlackListFragment view) {
        this.view = view;
    }
    ...
    public void releaseBlackState(final int position) {
        BlackOrAttentionBean item = view.list.get(position);
        final ShowDialog dialog = ShowDialog.showDialog(view.getContext(), view.getResources().getString(R.string.loading), true, null);
        Map<String, String> map = new HashMap<>();
        map.put("user_id", item.getUser_id() + "");
        map.put("type", "2");  //1 拉黑  2取消拉黑
        RetrofitManager.getInstance().create(ApiService.class)
